  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', animation: 'fadeIn 0.4s ease-out' }}>
      ...
      <div style={{ marginTop: '8px' }}>
        ...
        {isInsightsLoading ? (
          <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-light)' }}>Veriler analiz ediliyor...</div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(450px, 1fr))', gap: '24px', marginBottom: '24px' }}>
            <div className="glass-card" style={{ padding: '24px', borderRadius: '24px', border: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                <div style={{ padding: '8px', borderRadius: '10px', backgroundColor: 'rgba(82, 183, 136, 0.1)', color: 'var(--success)' }}>
                  <TrendingUp size={20} />
                </div>
                ...
                {insights?.best_sellers.map((p, idx) => (
                  <div key={p.id} style={{ 
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      <span style={{ fontWeight: 800, color: 'var(--primary-mid)', opacity: 0.5 }}>#{idx+1}</span>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '14px' }}>{p.name}</div>
                        <div style={{ fontSize: '12px', color: 'var(--text-light)' }}>{p.category}</div>
                      </div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontWeight: 800, color: 'var(--primary-dark)' }}>{p.total_sold} Satış</div>
                      <div style={{ fontSize: '12px', color: 'var(--success)' }}>{p.price} TL</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="glass-card" style={{ padding: '24px', borderRadius: '24px', border: '1px solid var(--border-color)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '16px' }}>
                <div style={{ padding: '8px', borderRadius: '10px', backgroundColor: 'rgba(230, 57, 70, 0.1)', color: 'var(--error)' }}>
                  <ShoppingBag size={20} />
                </div>
                ...
                {insights?.non_sellers.map((p) => (
                  <div key={p.id} style={{ 
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontWeight: 700, fontSize: '14px' }}>{p.name}</div>
                        <div style={{ fontSize: '12px', color: 'var(--text-light)' }}>Stok: {p.stock} | {p.price} TL</div>
                      </div>
                      <button 
                      ...
                      >
                        {isRecLoading[p.id] ? <RefreshCw size={14} className="spin-animation" /> : <Tag size={14} />}
                        Kampanya Önerisi
                      </button>
                    </div>
                    {campaignRec[p.id] && (
                      <div style={{ 
                        ...
                        <div style={{ fontWeight: 800, marginBottom: '4px', fontSize: '11px', textTransform: 'uppercase', color: 'var(--primary-mid)' }}>Koopilot Önerisi:</div>
                        {campaignRec[p.id]}
                      </div>
                    )}
                  </div>
      ...
      <div className="glass-card" style={{
        ...
        <h3 style={{ marginBottom: '20px', color: 'var(--text-dark)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <BarChart3 size={20} color="var(--primary-mid)" /> {labelPrefix} Niyet Dağılımı ve Trend
        </h3>
          ...
          <div style={{ height: '300px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                />
                <Legend verticalAlign="bottom" height={36} iconType="circle" />
                <Line type="monotone" dataKey="Adet" stroke="var(--primary-light)" strokeWidth={3} dot={{ r: 5, fill: 'var(--primary-dark)', strokeWidth: 2 }} activeDot={{ r: 7 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
